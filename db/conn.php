...
/**
 * Fournit tous les articles enregistrés dans la table produits
 * Si un id de catégorie est fourni, fournit seulement les articles de cette catégorie
 * @param bool $cat_id
 * @return array
 */
function get_produit_list($cat_id=false) {
    global $mysqli;
...
/**
 * Fournit un article de la table products à partir de son id
 * @param int $id
 * @return array
 */
function get_article() {
    global $mysqli;
// Sélectionner tous les articles (toutes les colonnes)
    $queryStr = 'SELECT * FROM products WHERE `id` = ';
// Execution de la requête (un select)
    $res = $mysqli->query($queryStr);
// Récupération des données
    $resultat = null;
